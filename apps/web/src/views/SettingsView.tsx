...
function TenantForm({ data, disabled, onSave }: { data: SettingsViewData['tenant']; disabled: boolean; onSave(input: Record<string, unknown> & { display_name: string; status: SettingsViewData['tenant']['status']; metadata: Record<string, unknown>; actor_id: string }): void }) {
  const [name, setName] = useState(data.display_name);
  const [status, setStatus] = useState(data.status);
  useEffect(() => { setName(data.display_name); setStatus(data.status); }, [data]);
  return <form className="settings-section" onSubmit={(event) => { event.preventDefault(); onSave({ display_name: name, status, metadata: data.metadata, actor_id: 'dashboard-operator' }); }}><SectionHeader title="Tenant" detail={data.slug} /><div className="form-grid"><label className="field"><span>Display name</span><input value={name} onChange={(event) => setName(event.target.value)} /></label><label className="field"><span>Status</span><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="active">Active</option><option value="suspended">Suspended</option><option value="archived">Archived</option></select></label></div><SaveButton disabled={disabled} /></form>;
}
